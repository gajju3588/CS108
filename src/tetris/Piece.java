...
public class Piece {
	...
	*/
	...
	public Piece computeNextRotation() {
        ...
        // shift right to make coordinate right.
        for (TPoint p : points)
            p.x -= minX;
	...
	*/
	public boolean equals(Object obj) {
        ...
        TPoint[] otherBody = Arrays.copyOf(other.body, other.body.length);
        TPoint[] thisBody = Arrays.copyOf(this.body, this.body.length);
        HashSet<TPoint> t1,t2;
        t1 = new HashSet<TPoint>();

        for (TPoint tPoint : otherBody) {
            t1.add(tPoint);
        }
        for (TPoint tPoint : thisBody) {
            if(!t1.contains(tPoint))
                return false;
        }
        return true;
